...
### Step 3: Write the rule (TDD order — tests first)

**Always write in this order:**

1. Write test file in `Tests/RulesTests/` first — violation cases, false-positive cases, edge cases
2. Run `swift test --filter <RuleName>` — all tests should fail (red)
3. Create `.swift` implementation file in `Sources/Rules/`
4. Register the rule in `RuleSet` in `Rules.swift`
...
If the user says "also catch X" or "that's not quite right", read the existing rule **and** test file before modifying. Always add or update tests first, confirm they fail (or capture the new expected behavior), then update the implementation.
