...
struct ScaffoldTests {
    ...
    func allFiles() async throws {
        try await FileManager.default.runInTemporaryDirectory { dir in
            ...
            #expect(fileManager.fileExists(atPath: "\(path)/Tests/RulesTests/RulesTests.swift"))
    ...
    @Test("RulesTests.swift contains SwiftASTLintTestSupport import")
    func testTargetContent() async throws {
        try await FileManager.default.runInTemporaryDirectory { dir in
            let path = dir.appendingPathComponent("TLinter").path(percentEncoded: false)
            try await Scaffold().generate(at: path, name: "TLinter")

            let content = try String(contentsOfFile: "\(path)/Tests/RulesTests/RulesTests.swift", encoding: .utf8)
            #expect(content.contains("import SwiftASTLintTestSupport"))
            #expect(content.contains("@testable import Rules"))
        }
    }

    @Test("removes auto-generated entry point that conflicts with main.swift")
    func removesAutoGeneratedEntryPoint() async throws {
        try await FileManager.default.runInTemporaryDirectory { dir in
            let path = dir.appendingPathComponent("CLinter").path(percentEncoded: false)
            try await Scaffold().generate(at: path, name: "CLinter")

            #expect(!FileManager.default.fileExists(atPath: "\(path)/Sources/swift-ast-lint/swift-ast-lint.swift"))
            #expect(FileManager.default.fileExists(atPath: "\(path)/Sources/swift-ast-lint/main.swift"))
        }
    }
